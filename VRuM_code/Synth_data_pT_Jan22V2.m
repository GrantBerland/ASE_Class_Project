function [B_tot_P1, B_ext_vec] = Synth_data_pT_Jan22V2(temp_noise,plot_flag)
...
for i = 1:3
    ...
    % coil contributions
    b1_vec(i,:) = (b1 + b1_noise).*cos(2*pi.*(fm1 + fm1_noise).*timeStamp)*e_b1(i);
    b2_vec(i,:) = (b2 + b2_noise).*cos(2*pi.*(fm2 + fm2_noise).*timeStamp)*e_b2(i);
    b3_vec(i,:) = (b3 + b3_noise).*cos(2*pi.*(fm3 + fm3_noise).*timeStamp)*e_b3(i);
end
